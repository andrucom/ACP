...
namespace backup
{
	...
	public:
		...
		void input(const fs::path path, const std::string folder_name, const fs::path pathend);

		void zip(const std::string& pathend);

		void copyDirectory(fs::path& source, const fs::path& destination, std::string origname);

		void createFolder(const fs::path& path, std::string folder_name, std::unordered_set<std::string> allowedNames, Settings settings, fs::path pathend);

		void createFolderWF(const fs::path& path, std::string folder_name, std::unordered_set<std::string> allowedNames, Settings settings, fs::path pathend);

		void OpenMainDir(const std::string path, const std::string pathend);
